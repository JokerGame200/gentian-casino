...
export default function Dashboard() {
  ...
  const sections = useMemo(() => {
    const providers = ['NovaPlay', 'StarForge', 'BlueWhale', 'MintLabs', 'AmberTech', 'LunarSpin'];
    const mk = (prefix, count, tag) =>
      Array.from({ length: count }).map((_, i) => ({
        id: `${prefix}-${i + 1}`,
        title: `${prefix} ${i + 1}`,
        provider: providers[(i + prefix.length) % providers.length],
        category: tag,
        image: TRANSPARENT_PX,
      }));

    return [
      { key: 'popular', title: 'Most Popular', tag: 'Popular', items: mk('Popular', 24, 'Popular') },
      { key: 'exclusive', title: 'Only on Next2Win', tag: 'Exclusive', items: mk('Exclusive', 18, 'Exclusive') },
      { key: 'new', title: 'New Releases', tag: 'New', items: mk('New', 20, 'New') },
      { key: 'provider', title: 'Best of Provider', tag: 'Provider', items: mk('Provider Pick', 20, 'Provider') },
      { key: 'trending', title: 'Trending', tag: 'Trending', items: mk('Trending', 20, 'Trending') },
      { key: 'buy', title: 'Buy Bonus Slots', tag: 'Buy Bonus', items: mk('Buy Bonus', 20, 'Buy Bonus') },
      { key: 'crash', title: 'Crash & Instant', tag: 'Crash', items: mk('Instant', 16, 'Crash') },
      { key: 'collections', title: 'Collections', tag: 'Themes', items: mk('Theme', 15, 'Themes') },
      { key: 'jackpots', title: 'Jackpots', tag: 'Jackpots', items: mk('Jackpot', 12, 'Jackpots') },
      { key: 'arcade', title: 'Arcade Games', tag: 'Arcade', items: mk('Arcade', 12, 'Arcade') },
      { key: 'table', title: 'Table Games', tag: 'Table', items: mk('Table', 12, 'Table') },
    ];
  }, []);

  const filteredSections = useMemo(() => {
    if (selectedCat === 'All') return sections;
    const map = {
      Popular: ['popular'],
      New: ['new'],
      Exclusive: ['exclusive'],
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
          ...
          <div className="space-y-10 mt-8">
            {filteredSections.map((s) => (
              <SectionCarousel key={s.key} title={s.title} items={s.items} />
            ))}
          </div>
        </main>
        <Footer />
...
function Header({ user, initials, balanceText, selectedCat, setSelectedCat }) {
  ...
  const categories = [
    'All', 'Popular', 'New', 'Exclusive', 'Buy Bonus', 'Crash', 'Table', 'Arcade', 'Provider', 'Themes', 'Jackpots',
  ];
        ...
        <div className="fixed inset-0 z-50 sm:hidden">
          ...
          <div className="absolute bottom-0 inset-x-0 rounded-t-2xl bg-[#0c1e31] border-t border-white/10 p-4 max-h-[70vh]">
            ...
            <div className="grid grid-cols-2 gap-2">
              {['All','Popular','New','Exclusive','Buy Bonus','Crash','Table','Arcade','Provider','Themes','Jackpots'].map(cat => (
                <button
                  key={cat}
  ...
  return (
    <section aria-label={title}>
      ...
      <div className="relative group">
        ...
        >
          {items.map((g) => <LazyGameCard key={g.id} game={g} />)}
        </div>
  ...
  useEffect(() => {
    ...
    if ('IntersectionObserver' in window) {
      obs = new IntersectionObserver(
        (e) => {
          if (e[0]?.isIntersecting) { setNear(true); obs.disconnect(); }
        },
        { rootMargin: '400px' }
      );
          ...
          <div className="absolute inset-0 opacity-0 group-hover/card:opacity-100 transition-opacity">
            ...
            <div className="absolute inset-0 grid place-items-center">
              <div className="flex gap-2">
                <button className="px-3 py-1.5 rounded-lg bg-white text-black text-sm font-semibold hover:brightness-95">Play</button>
                <button className="px-3 py-1.5 rounded-lg bg-white/10 border border-white/20 text-sm hover:bg-white/15">Demo</button>
              </div>
            </div>
          </div>
        )}
...
function HeartIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M12 21s-7.5-4.6-9.5-8.6C1.2 9.8 3 6.8 6 6.3c1.8-.3 3.4.5 4 1.7.6-1.2 2.2-2 4-1.7 3 .5 4.8 3.5 3.5 6.1C19.5 16.4 12 21 12 21z" stroke="currentColor" strokeWidth="1.5" />
    </svg>
  );
}
