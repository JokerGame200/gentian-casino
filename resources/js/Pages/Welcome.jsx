...
/* --- Suchfeld (stabiler Fokus, kein Remount) — auf Modulebene! --- */
const SearchInput = memo(function SearchInput({ value, onChange, placeholder }) {
  ...
  useLayoutEffect(() => {
    // Falls vor dem Render fokussiert: Fokus halten
...
export default function Welcome() {
  ...
  // ---- Normalisierung + Dedupe (viel robuster)
  const normalizeName = (s) => String(s ?? '')
    .toLowerCase()
    .replace(/[™®©]/g, '')
    .replace(/\b(deluxe|classic|cash\s*link|dx|hd|xxl|gold|megaways|mega\s*ways)\b/g, '') // typische Zusätze raus
    .replace(/[-_.:/\\]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  ...
  const pickImage = (g) => {
    ...
    for (const x of cands) {
      if (isMeaningfulUrl(x)) return String(x);
    }
  ...
  const buildKeyCandidates = (g) => {
    const idCands = [
      g?.uid, g?.uuid, g?.game_id, g?.sys_id, g?.id, g?.slug, g?.code, g?.key, g?.external_id
    ].filter(v => v != null);
  ...
  const hasBetterThumb = (a, b) => {
    ...
    return ia.length >= ib.length; // heuristisch
  };
  ...
  const normalizeAndDedupe = (list) => {
    ...
    for (const raw of list) {
      ...
      if (!bucketKey) {
        for (const k of keys) {
          if (!reserved.has(k)) { bucketKey = k; break; }
        }
      }
      if (!bucketKey) {
        bucketKey = provN && nameN ? `provname:${provN}#${nameN}` : (nameN || `rand:${Math.random().toString(36).slice(2)}`);
      }
  ...
  // --- Kategorien & Such-States (pro Reiter eigener Query-Text)
  const [selectedCat, setSelectedCat] = useState('All');
  const [queries, setQueries] = useState({}); // { [category]: string }
  ...
  const LOAD_STEP = 20;
  const [showCount, setShowCount] = useState(LOAD_STEP);
  useEffect(() => { setShowCount(LOAD_STEP); }, [selectedCat, q]);
  ...
  const filteredGames = useMemo(() => {
    ...
    // Sicherheits-Dedupe nach Filter (falls API-Felder je Kategorie differieren)
  ...
  const searchPlaceholder = selectedCat === 'All'
    ? 'Search games or providers…'
    : `Search in ${selectedCat}…`;
            ...
            <div className="space-y-10 mt-8">
              {selectedCat === 'All' ? (
                ...
                <SectionGrid
                  ...
                  onLoadMore={() => setShowCount(c => c + LOAD_STEP)}
                  onPlay={openGame}
                  rightNode={
                    <SearchInput
                      value={q}
...
function Header({ user, initials, balanceText, selectedCat, setSelectedCat }) {
  ...
  const categories = [
    'All','Popular','Exclusive','Arcade','Novomatic','Amatic','Pragmatic','EGT','Wazdan','Aristocrat'
  ];
            ...
            {categories.map(cat => (
              <button
                ...
                className={[
                  ...
                  selectedCat === cat ? 'bg-cyan-500 text-black shadow' : 'bg-white/5 hover:bg-white/10 text-white'
                ].join(' ')}
              >
                {cat}
              </button>
...
function SectionGrid({ title, items, visibleCount, onLoadMore, onPlay, rightNode = null }) {
  const visible = items.slice(0, Math.max(0, visibleCount || 0));
  ...
  return (
    <section aria-label={title}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-3">
        {visible.map((g) => (
          <GameCard key={gameReactKey(g)} game={g} onPlay={openGameProxy(onPlay)} variant="grid" />
        ))}
      </div>
      ...
      {hasMore && (
        <div className="mt-5 flex justify-center">
          ...
          >
            Mehr laden ({items.length - visible.length} weitere)
          </button>
        </div>
      )}
    </section>
...
function GameCard({ game, onPlay, variant = 'carousel' }) {
  ...
  return (
    <article className={containerCls} aria-label={title}>
      <div className="relative aspect-video rounded-xl overflow-hidden group/card bg-gradient-to-br from-cyan-400/20 via-indigo-400/15 to-emerald-400/20 border border-white/10">
        {/* Thumb - ohne IntersectionObserver -> immer sichtbar */}
